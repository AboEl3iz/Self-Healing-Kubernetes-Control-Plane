...
import (
	...
	"github.com/karim-aboelaiz/selfheal-cp/internal/tui/app"
	"github.com/karim-aboelaiz/selfheal-cp/internal/tui/config"
	"github.com/karim-aboelaiz/selfheal-cp/pkg/cgroup"
	"github.com/karim-aboelaiz/selfheal-cp/pkg/collector"
	"github.com/karim-aboelaiz/selfheal-cp/pkg/graph"
)

func main() {
	// ── CLI flags (backward compatible with original) ─────────────────────────
	intervalFlag := flag.Duration("interval", 0, "data poll interval (overrides config)")
	cpuBPF := flag.String("cpu-bpf", "ebpf/probes/cpu.o", "CPU BPF object (M1)")
	memBPF := flag.String("mem-bpf", "ebpf/probes/memory.o", "Memory BPF object (M2)")
	ioBPF := flag.String("io-bpf", "ebpf/probes/io.o", "I/O BPF object (M3)")
	netBPF := flag.String("net-bpf", "ebpf/probes/network.o", "Network BPF object (M4)")
	sysBPF := flag.String("sys-bpf", "ebpf/probes/syscall.o", "Syscall BPF object (M6)")
	_ = flag.String("lineage-bpf", "ebpf/security/lineage.o", "Lineage BPF object (Phase 1) - ignored in favor of exec extraction")
	execBPF := flag.String("exec-bpf", "ebpf/security/exec.o", "Exec BPF object (Phase 2)")
	dnsBPF := flag.String("dns-bpf", "ebpf/security/dns.o", "DNS BPF object (Phase 3)")
	privescBPF := flag.String("privesc-bpf", "ebpf/security/privesc.o", "PrivEsc BPF object (Phase 4)")
	escapeBPF := flag.String("escape-bpf", "ebpf/security/escape.o", "Escape BPF object (Phase 5)")
